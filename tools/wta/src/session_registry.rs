...
pub struct WtaMeta {
    ...
    /// Session id of the terminal surface that owns the chat binding. During
    /// migration this may equal `pane_session_id`, but it is intentionally a
    /// separate field so an agent-pane helper can follow another surface.
    pub terminal_session_id: Option<String>,
    /// Stable native-tab/workspace identity.
    pub workspace_id: Option<String>,
    /// Pane-local surface identity advertised by Terminal Protocol.
    pub surface_id: Option<String>,
    /// Monotonic focus generation, serialized as a decimal string for
    /// backwards-compatible `_meta` handling.
    pub focus_generation: Option<String>,
    ...
    /// Canonical ComputeTarget id paired with `agent_source=ssh`.
    pub ssh_target: Option<String>,
    /// Stable wta-node runtime paired with `agent_source=ssh`.
...
impl WtaMeta {
    ...
    pub fn is_empty(&self) -> bool {
        ...
        blank(&self.pane_session_id)
            && blank(&self.terminal_session_id)
            && blank(&self.workspace_id)
            && blank(&self.surface_id)
            && blank(&self.focus_generation)
            && blank(&self.agent_cmd)
            && blank(&self.agent_id)
            && blank(&self.model)
            && blank(&self.agent_source)
            && blank(&self.wsl_distro)
            && blank(&self.ssh_target)
            && blank(&self.remote_session_id)
...
pub fn extract_wta_meta(meta: &mut Option<acp::schema::v1::Meta>) -> WtaMeta {
    ...
    WtaMeta {
        ...
        terminal_session_id: str_field("terminal_session_id"),
        workspace_id: str_field("workspace_id"),
        surface_id: str_field("surface_id"),
        focus_generation: str_field("focus_generation"),
        ...
        ssh_target: str_field("ssh_target"),
        remote_session_id: str_field("remote_session_id"),
...
pub fn inject_wta_meta(meta: &mut Option<acp::schema::v1::Meta>, wta: &WtaMeta) {
    ...
    put("terminal_session_id", &wta.terminal_session_id);
    put("workspace_id", &wta.workspace_id);
    put("surface_id", &wta.surface_id);
    put("focus_generation", &wta.focus_generation);
    put("agent_cmd", &wta.agent_cmd);
    put("agent_id", &wta.agent_id);
    put("model", &wta.model);
    put("agent_source", &wta.agent_source);
    put("wsl_distro", &wta.wsl_distro);
    put("ssh_target", &wta.ssh_target);
    put("remote_session_id", &wta.remote_session_id);
...
mod tests {
    ...
    fn inject_then_extract_is_identity() {
        let original = WtaMeta {
            pane_session_id: Some("pane-X".to_string()),
            terminal_session_id: Some("terminal-X".to_string()),
            workspace_id: Some("workspace-X".to_string()),
            surface_id: Some("surface-X".to_string()),
            focus_generation: Some("42".to_string()),
