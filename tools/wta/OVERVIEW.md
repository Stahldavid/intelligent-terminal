...
## Managed remote runtime contracts

The compute control plane separates placement from runtime identity:

```text
ComputeTarget -> ExecutionEnvironment -> AccessEndpoint
                                    \-> EnvironmentConnectionSupervisor
```

`ComputeTarget` carries trust, capacity and cost. `ExecutionEnvironment`
carries stable runtime/protocol/OS/architecture identity. One connection
supervisor owns retry/backoff and endpoint selection; SSH/stdio is the current
bootstrap and fallback. Public/overlay endpoint kinds remain fail-closed.

Remote files require a persisted `RemoteFileRootPolicy`. Ordinary clients see
only an opaque root ID, label and capabilities and operate with relative paths.
The old path/HOME download route is disabled; use `wta compute file download`
with an authorized workspace root.
...
Terminal spawns one **`wta-master`** singleton that owns a lazy pool of trusted
ACP adapter connections, and one **`wta-helper`** per agent pane that renders
the compatibility TUI and talks ACP to master over a named pipe. A third,
stateless role is the **CLI helpers** (`wta list-panes`,
`wta capture-pane`, …) used for one-shot WT control.
...
1. Resolves a helper's declared agent ID through the trusted registry and
   allowlist, then lazily starts or reuses that adapter's subprocess and wraps
   stdio in an `acp::ClientSideConnection` — master is the *client* of each
   adapter.
...
| **Master** | `src/master/mod.rs` | ACP multiplexer singleton: owns the lazy trusted-adapter pool, serves helpers over the pipe, routes per-session notifications |
...
- **master ↔ agent adapter** (stdio): master is the ACP **client**; it lazily
  starts and owns the selected trusted adapter.
...
| ACP protocol | agent-client-protocol 1.2.0 |
...
| **wta-master** | `wta.exe --master` | Spawned once by `SharedWta` | Owns the lazy adapter pool; multiplexes ACP sessions for all helpers |
| **wta-helper** | `wta.exe --connect-master` | One per agent pane | TUI + per-pane side effects; ACP client of master |
| **Agent adapter** | `copilot`, `claude`, `gemini`, `codex` | Spawned lazily per trusted adapter key | AI runtime; reused by helpers selecting that adapter |
...
- One master serves all panes and windows. It reuses one adapter process for
  helpers with the same trusted command and can host different adapters in
  parallel. A helper's `session/new` round-trips to its selected adapter;
  `initialize` is cached per adapter.
